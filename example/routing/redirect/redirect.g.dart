...
//Handler for getBookById
    ...
    if (match) {
      return await Interceptor.chain(ctx, _internal.getBookById, routes[0]);
    }

//Handler for redirectMe
    match = routes[1].match(ctx.path, ctx.method, prefix, ctx.pathParams);
    if (match) {
      return await Interceptor.chain(ctx, _internal.redirectMe, routes[1]);
    }
...
//Handler for redirectWithQuery
    ...
    if (match) {
      return await Interceptor.chain(
          ctx, _internal.redirectWithQuery, routes[2]);
    }
